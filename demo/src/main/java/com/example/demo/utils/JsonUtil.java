...
public class JsonUtil {
    ...
    public static <T> T fromJson(String json, TypeToken<T> token, String datePattern) {
        ...
        builder.setDateFormat(datePattern);
        builder.registerTypeAdapter(Map.class, new MapTypeAdapter());
        builder.registerTypeAdapter(Double.class, new JsonSerializer<Double>() {
            @Override
            public JsonElement serialize(final Double src, final Type typeOfSrc, final JsonSerializationContext context) {
                BigDecimal value = BigDecimal.valueOf(src);
    ...
    public static <T> T fromJson(String json, Class<T> clazz, String datePattern) {
        ...
        try {
            ...
            log.warn(" 无法转换为 " + clazz.getName() + " 对象!----1300001", ex);
            log.warn(" json数据无法转换为 " + clazz.getName() + " 对象!, 数据为: {}", json);
            return null;
        } catch (IllegalStateException ex) {
            log.warn(" 无法转换为 " + clazz.getName() + " 对象!----1300003", ex);
            log.warn(" json数据无法转换为 " + clazz.getName() + " 对象!, 数据为: {}", json);
            return null;
        } catch (ClassCastException ex) {
            log.warn(" 无法转换为 " + clazz.getName() + " 对象!----1300004", ex);
            log.warn(" json数据无法转换为 " + clazz.getName() + " 对象!, 数据为: {}", json);
            return null;
        } catch (Exception ex) {
            log.error("[" + json + "] 无法转换为 " + clazz.getName() + " 对象!----1300002", ex);
            return null;
        }
    }
